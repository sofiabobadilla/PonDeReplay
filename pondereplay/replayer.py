...
class TransactionReplayer:
    ...
    def sanity_check(
        ...
        if verbose:
            print(f"[*] Fetching original contract bytecode at block {block_number - 1}...")
        
        # Get original bytecode at the block before the transaction
        try:
            original_bytecode = self.w3.eth.get_code(contract_address, block_identifier=block_number - 1)
        ...
        if verbose:
            print(f"[*] Replaying transaction with ORIGINAL bytecode...")
    ...
    def _compare_results(self, result: ReplayResult, receipt: dict, verbose: bool = False) -> bool:
        """
        ...
        if not result.success:
            if verbose:
                print(f"[!] Replay failed, cannot compare with original")
            return False
    ...
    def _replay_with_web3(
        ...
        if verbose:
            print(f"[*] Preparing to replay with patched bytecode...")
        ...
        try:
            ...
            if verbose:
                ...
                print(f"[*] New bytecode length: {len(new_bytecode)} chars ({len(new_bytecode) // 2} bytes)")
    ...
    def _replay_with_anvil(
        ...
        """
        Replay using anvil fork and bytecode patching.
        This requires anvil to be running and configured.
        (Alternative implementation if eth_call state overrides don't work)
        """
        import subprocess
        
        if verbose:
            print(f"[*] Starting anvil fork at block {block_number}...")
        ...
        try:
            ...
            anvil_process = subprocess.Popen(
                [
                    ...
                    "--fork-block-number", str(block_number - 1),
                    "--port", "8545",
                ],
            ...
            # Set bytecode
            w3_local.eth.set_code(contract_address, new_bytecode)
            
            if verbose:
                print(f"[*] Bytecode patched")
            ...
            result = w3_local.eth.call(tx_data)
